...
//It is the strongest possible check whether any changes to the program affect the output many timesteps later.
#include <iostream>
...
bool check_level_data(LevelData<FArrayBox>& lev_dat_1, LevelData<FArrayBox>& lev_dat_2)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        {
            IntVect iv = bit();
            for (int icomp=0; icomp < 20 /*lev_dat_1.nComp() TODO REMOVE THIS HARDCODE*/; ++icomp)
            {
                Real difference = fab_1(iv, icomp) - fab_2(iv, icomp);
                if ( difference != 0 )
                {
                    abort_counter++;
                    pout () << "Values at position " << iv << " in component " << icomp <<
                        " disagree by " << difference << endl;
                    pout () << "val 1: " << fab_1(iv, icomp);
                    pout () << " val 2: " << fab_2(iv, icomp) << endl;
                    disagree = true;
                    if (abort_counter > 50)
                    {
                        pout() << "Too many problems on this level. Giving up and going to the next... " << endl;
...
int main(int argc ,char* argv[])
{
    ...
    if (num_procs > 1) MayDay::Error("This little tool is not suitable for more than 1 rank (although it wouldn't be hard to change this...)");
#ifdef CH_AIX
    H5dont_atexit();
#endif
#endif
    ...
    // read max level
    if (header_1.m_int.find("max_level") == header_1.m_int.end()) MayDay::Error("File 1 does not contain max_level");
    int max_level_1 = header_1.m_int ["max_level"];

    if (header_2.m_int.find("max_level") == header_2.m_int.end()) MayDay::Error("File 1 does not contain max_level");
    ...
    for (int ilev=0; ilev<=max_comparison_level; ++ilev)
    {
        ...
        Real dx_1; Real dt_1; Real time_1; Box box_1; int ref_ratio_1;
        readLevel(handle_1, ilev, level_data_1, dx_1, dt_1, time_1, box_1, ref_ratio_1, level_data_1.interval());

        //Read file_2
        LevelData<FArrayBox> level_data_2;
        Real dx_2; Real dt_2; Real time_2; Box box_2; int ref_ratio_2;
        readLevel(handle_2, ilev, level_data_2, dx_2, dt_2, time_2, box_2, ref_ratio_2, level_data_2.interval());
        ...
        //level_data_1 and level_data_2 might have different box layouts so need to do a copy
        LevelData<FArrayBox> level_data_2_copy(level_data_1.disjointBoxLayout(), level_data_1.nComp(), level_data_1.ghostVect());

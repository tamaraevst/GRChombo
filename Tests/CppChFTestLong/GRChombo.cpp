...
//#pragma omp parallel for default(shared) schedule(guided)
  for(int ibox = 0; ibox < nbox; ++ibox)
  {
    ...
    //Prepare another FArrayBox
    FArrayBox chf_rhs_fab(b, s_num_comps);

    CCZ4(m_p.ccz4Params, m_dx, m_p.sigma).execute(soln_fab, rhs_fab);

    FORT_GETCCZ4CRHSF(CHF_FRA1(chf_rhs_fab,c_chi),
                    CHF_FRAn(chf_rhs_fab,c_h,s_num_comps_h),
                    CHF_FRA1(chf_rhs_fab,c_K),
                    CHF_FRAn(chf_rhs_fab,c_A,s_num_comps_A),
                    CHF_FRA1(chf_rhs_fab,c_Theta),
                    CHF_FRAn(chf_rhs_fab,c_Gamma,s_num_comps_Gamma),
                    CHF_FRA1(chf_rhs_fab,c_lapse),
                    CHF_FRAn(chf_rhs_fab,c_shift,s_num_comps_shift),
                    CHF_FRAn(chf_rhs_fab,c_B,s_num_comps_B),

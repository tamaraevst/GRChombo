...
double StarTracker::find_centre(int direction)
{
    int success;
    double delta;
    ...
    for (int i = 0; i < m_points; i++)
    {
        ...
        if (direction == 1 )
        {	
            x_coords[i] = m_star_coords[0];
            y_coords[i] = m_star_coords[1] + delta;
            z_coords[i] = m_star_coords[2];
        }
        ...
        if (direction == 2 )
        {
            x_coords[i] = m_star_coords[0];
            y_coords[i] = m_star_coords[1];
            z_coords[i] = m_star_coords[2] + delta;
        }
...
void StarTracker::find_max_min(int direction)
{
    double delta;
                               ...
                               bool a_first_step)
{
    ...
    std::vector<string> header_line(3.);

    header_line[0] = "x" + to_string(1);
    header_line[1] = "y" + to_string(1);
    header_line[2] = "z" + to_string(1);
    ...
    std::vector<double> m_star_coords_vec = {m_star_coords[0], m_star_coords[1], m_star_coords[2]};

    star_centre_file.write_time_data_line(m_star_coords_vec);
}
                                           ...
                                           bool a_first_step)
{
    ...
    bool length_match = data_line.size() == CH_SPACEDIM;

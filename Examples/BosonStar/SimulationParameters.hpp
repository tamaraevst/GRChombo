...
public:
    ...
    void readParams(GRParmParse &pp)
    {
        ...
        pp.load("central_amplitude_CSF", bosonstar_params.central_amplitude_CSF);
        pp.load("abs_error", bosonstar_params.abs_error, 1.0e-14);
        pp.load("rel_error", bosonstar_params.rel_error, 1.0e-14);
        pp.load("initial_step_size", bosonstar_params.initial_step_size, 0.015625);
        pp.load("max_radius", bosonstar_params.max_radius, 256.);
        pp.load("binary_search_tol", bosonstar_params.binary_search_tol, 1.0e-15);
        pp.load("max_binary_search_iter", bosonstar_params.max_binary_search_iter, 1000);
        pp.load("star_centre", bosonstar_params.star_centre, {0.5*L, 0.5*L, 0.5*L});

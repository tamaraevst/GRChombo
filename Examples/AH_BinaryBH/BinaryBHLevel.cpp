...
#ifdef USE_AHFINDER
    if (m_p.AH_activate && m_level == m_p.AH_params.level && m_time > 0.0)
    {
        if (m_p.use_punctures_for_ah_centers && m_p.track_punctures)
        {
            auto &ahs = m_tp_amr.m_apparent_horizons;
            for (int iah = 0; iah < ahs.size(); ++iah)
            {
                // set non-merger apparent horizon centers to punctures
                if (ahs[iah]->is_merger())
                    continue;
                ahs[iah]->set_center(
                    m_tp_amr.m_puncture_tracker.get_puncture_coords()[iah]);
            }
        }
        pout() << "AHFinder::solve on level " << m_level << endl;
        AHFinder::solve(m_tp_amr, m_dt, m_time, m_restart_time);
    }
#endif

  ...
  public:
    ...
    void execute_tracking(GRAMR &a_gramr) const
    {
        ...
        for (int ipuncture=0; ipuncture < m_num_punctures; ipuncture++)
        {
            ...
            interp_y[ipuncture] = new_puncture_coords[CH_SPACEDIM*ipuncture + 1];
            interp_z[ipuncture] = new_puncture_coords[CH_SPACEDIM*ipuncture + 2];
        }
        ...
        for (int ipuncture=0; ipuncture < m_num_punctures; ipuncture++)
        {
            new_puncture_coords[CH_SPACEDIM*ipuncture    ] += -m_dt * interp_shift1[ipuncture];
            new_puncture_coords[CH_SPACEDIM*ipuncture + 1] += -m_dt * interp_shift2[ipuncture];
            new_puncture_coords[CH_SPACEDIM*ipuncture + 2] += -m_dt * interp_shift3[ipuncture];
        }
        a_gramr.set_puncture_coords(new_puncture_coords);

        // print them out
        std::string extraction_filename = m_checkpoint_prefix + "_Punctures.txt";
        SmallDataIO extraction_file(extraction_filename, m_dt, m_time, m_restart_time,
                                       SmallDataIO::APPEND);

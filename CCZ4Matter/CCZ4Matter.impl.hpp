...
CCZ4Matter<matter_t>::CCZ4Matter(const FABDriverBase& driver, params_t params, double dx, double sigma,
		int formulation, double G_Newton) :
    CCZ4(driver, params, dx, sigma, formulation, 0.0), //Explicitly no cosmological constant
		m_G_Newton (G_Newton)
{}

//TODO Do I need to inline??

template <class matter_t>
template <class data_t>
void
CCZ4Matter<matter_t>::compute(int ix, int iy, int iz)
{
    vars_t<data_t> CCZ4_vars;
    m_driver.local_vars(CCZ4_vars);  //copy data from chombo gridpoint into local variables

    vars_t< tensor<1, data_t> > CCZ4_d1;
    FOR1(idir) m_deriv.diff1(CCZ4_d1, idir); //work out first derivatives of variables on grid

    vars_t< tensor<2,data_t> > CCZ4_d2;
    // Repeated derivatives
    FOR1(idir) m_deriv.diff2(CCZ4_d2, idir);  //work out second derivatives of variables on grid
    // Mixed derivatives
    ...
    // Call CCZ4 RHS
    vars_t<data_t> CCZ4_rhs = rhs_equation(CCZ4_vars, CCZ4_d1, CCZ4_d2, CCZ4_advec); //work out RHS without matter
    ...
    typename matter_t::vars_t<data_t> matter_rhs = matter_rhs_equation(vars, d1, d2, advec); //work out RHS matter terms
    ...
    FOR1(idir) m_deriv.add_dissipation(rhs_total, m_sigma, idir); //add dissipation to RHS
...
template <class matter_t>
template <class data_t>
typename matter_t::vars_t<data_t>
CCZ4Matter<matter_t>::matter_rhs_equation(
...
//    const data_t chi_regularised = simd_max(1e-6, vars.chi);
    ...
    FOR2(i,j)
		{
			add_rhs.A[i][j] = - 8.0*M_PI*m_G_Newton*vars.chi*vars.lapse*vars.lapse*Sij_TF[i][j];
		}

    FOR2(i,j)
    {
     		add_rhs.Gamma[i] = - 16.0*M_PI*m_G_Newton*vars.lapse*h_UU[i][j]*emtensor.Si[j];
		}

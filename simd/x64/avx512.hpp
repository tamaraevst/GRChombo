...
template <>
struct simd<double> : public simd_base<double>
{
    ...
    ALWAYS_INLINE
    simd() :
        simd_base<double> (_mm512_undefined_pd())
    {}
    
    ALWAYS_INLINE
    simd(double x) :
        simd_base<double> (_mm512_set1_pd(x))
    {}
    ...
    friend ALWAYS_INLINE
    mask_t simd_compare_lt(const simd& a, const simd& b)
    {
        return _mm512_cmp_pd_mask(a, b, _CMP_LT_OQ);
    }

    friend ALWAYS_INLINE
    mask_t simd_compare_gt(const simd& a, const simd& b)
    {
        return _mm512_cmp_pd_mask(a, b, _CMP_GT_OQ);
    }

    friend ALWAYS_INLINE
    simd simd_min(const simd& a, const simd& b)
    {
...
template <>
struct simd<float> : public simd_base<float>
{
    ...
    ALWAYS_INLINE
    simd() :
        simd_base<float> (_mm512_undefined_ps())
    {}
    
    ALWAYS_INLINE
    simd(float x) :
        simd_base<float> (_mm512_set1_ps(x))
    {}

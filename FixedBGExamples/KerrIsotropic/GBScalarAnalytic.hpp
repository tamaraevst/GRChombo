...
class GBScalarAnalytic
{
  ...
  // using CCZ4 = CCZ4RHS<MovingPunctureGauge, FourthOrderDerivatives>;
  
  // template <class data_t>
  // using CCZ4Vars = typename CCZ4::template Vars<data_t>;


    GBScalarAnalytic(std::array<double, CH_SPACEDIM> a_center, double a_dx, double a_mass) : m_center(a_center), m_dx(a_dx), m_mass(a_mass)
    {
    }

...
// This is the base template,i.e. the fallback for when there is no SIMD support for a data type.
// The fallback is to just not use simd.
// The datatypes that have simd support correspond to template specialisations, which are defined in the
...
// The fallback is to just not use simd. Hence, all the member function definitions of
// simd below are pretty trivial.
// The datatypes that have simd support correspond to template specialisations, which are defined in the
// architecture-specific files and simd_base.hpp
template <typename t>
struct simd
...
//We have defined various simd-specific calls (simd_compare_lt, simd_compare_gt, min, max etc.)
//For simd<t> these are defined in the various architecture-specific implementations.
//Here, we make sure that the same function calls also work when simd is switched off.
//(e.g. simd_compare_lt(double, double) must work but needs to be implemented
template <typename t>
...
template <typename t>
ALWAYS_INLINE
bool simd_compare_lt(const t& a, const t& b)
{
    return a < b;
}
...
template <typename t>
ALWAYS_INLINE
bool simd_compare_gt(const t& a, const t& b)
{
    return a > b;
}

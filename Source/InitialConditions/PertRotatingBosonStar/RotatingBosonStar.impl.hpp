...
{   
    ...
    vars.phi_Re += A_val * (cos(phase_) + m_params_RotatingBosonStar.pertA * cos(20. * phase_));
    vars.phi_Im += A_val * (sin(phase_) + m_params_RotatingBosonStar.pertA * sin(20. * phase_));
    ...
    KLL[0][0] = (l_val*sin(2.*phi)*(cos(theta) * sin(theta) * dthomega_val+sin(theta)*sin(theta)*(-omega_val + r*dromega_val)))/(2*f_val*r*lapse);
    KLL[0][1] = (l_val*cos(2.*phi)*(-cos(theta) * sin(theta) * dthomega_val+sin(theta)*sin(theta)*(omega_val - r*dromega_val)))/(2*f_val*r*lapse);
    KLL[1][0] = KLL[0][1];
    KLL[1][1] = -KLL[0][0];
    KLL[0][2] = (-l_val*sin(phi)*(sin(theta)*sin(theta)*dthomega_val + cos(theta)*sin(theta)*(omega_val - r*dromega_val)))/(2.*f_val*r*lapse);
    KLL[2][0] = KLL[0][2];
    KLL[1][2] = (l_val*cos(phi)*(sin(theta)*sin(theta)*dthomega_val + cos(theta)*sin(theta)*(omega_val - r*dromega_val)))/(2.*f_val*r*lapse);
    KLL[2][1] = KLL[1][2];
    KLL[2][2] = 0;

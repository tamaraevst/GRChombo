...
namespace BoxLoops
{
    ...
    void innermost_loop(const ComputePack<compute_ts...>& compute_pack, const BoxPointers& box_pointers,
    ...
    void loop(const ComputePack<compute_ts...>& compute_pack, const FArrayBox& in, FArrayBox& out, const Box & loop_box, simd_info... info);
    ...
    loop(compute_t compute_class, const FArrayBox& in, FArrayBox& out, const Box & loop_box, simd_info... info);

    ///Performs loop insde the whole box of 'out' and calls compute(...) for all compute classes in the compute_pack
    ///with input FArrayBox 'in' and output FArrayBox 'out'.
    template <typename... compute_ts, typename... simd_info>
    void loop(const ComputePack<compute_ts...>& compute_pack, const FArrayBox& in, FArrayBox& out, simd_info... info); //Uses out.box() as loop_box

    ///Same as above but for only one compute class (rather than a pack of them)
    template <typename compute_t, typename... simd_info>
    std::enable_if_t<!is_compute_pack<compute_t>::value, void>
    loop(compute_t compute_class, const FArrayBox& in, FArrayBox& out, simd_info... info); //Uses out.box() as loop_box

    ///Performs loop over all boxes and inside all boxes of the LevelData 'out' and calls compute(...) for all compute
    //classes in the compute_pack with input data taken from 'in' and output written to 'out'
    ...
    template <typename... compute_ts, typename... simd_info>
    void loop(const ComputePack<compute_ts...>& compute_pack, const LevelData<FArrayBox>& in, LevelData<FArrayBox>& out, bool fill_ghosts, simd_info... info);

    ///Same as above but for only one compute class (rather than a pack of them)
    template <typename compute_t, typename... simd_info>
    std::enable_if_t<!is_compute_pack<compute_t>::value, void>
    loop(compute_t compute_class, const LevelData<FArrayBox>& in, LevelData<FArrayBox>& out, bool fill_ghosts, simd_info... info);
}

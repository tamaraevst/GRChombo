...
/*!
   ...
   evolution terms. In this case, a scalar field, the matter elements are phi
   and (minus) its conjugate momentum, Pi. It is templated over a potential
   function potential_t which the user must specify in a class, although a
   default is provided which sets dVdphi and V_of_phi to zero. It assumes
   minimal coupling of the field to gravity. \sa CCZ4Matter(),
   ConstraintsMatter()
*/
  ...
  public:
    ...
    emtensor_t<data_t> compute_emtensor(
        ...
        const tensor<3, data_t>
            &chris_ULL //!< the conformal christoffel symbol in ULL form.
        ) const;
    ...
    void add_matter_rhs(
        vars_t<data_t>
            &total_rhs, //!< contains the value of the RHS terms for all vars.
        const vars_t<data_t>
            &vars, //!< the value of the variables at the point.
        const vars_t<tensor<1, data_t>>
            &d1, //!< the value of the first derivatives of the variables.
        ...
        const vars_t<data_t>
            &advec //!< the value of the advection terms beta^i d_i(var).
        ) const;
};
